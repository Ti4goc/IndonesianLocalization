...
namespace IndonesianLocale
{
    public class Mod : IMod
    {
        public static ILog log = LogManager
            .GetLogger($"{nameof(IndonesianLocale)}.{nameof(Mod)}")
            .SetShowsErrorsInUI(false);

        private const string CrowdinNotificationId =
            "indonesian-locale-crowdin-notification";

        private const string CrowdinUrl =
            "https://crowdin.com/project/community-localizations-csii";

        private const string CrowdinThumbnailUri =
            "assetdb://user/Mods/IndonesianLocale/id.jpg";

        private static bool _crowdinNotificationShown;

        public void OnLoad(UpdateSystem updateSystem)
        {
            log.Info("=== Indonesian Locale Mod Loading ===");
        ...
        private bool TryShowCrowdinNotification()
        {
            ...
            try
            {
                ...
                NotificationSystem.Push(
                    ...
                    title: "Masalah terjemahan, inkonsistensi, kesalahan, atau hanya ingin membantu?",
                    text: "Klik di sini untuk membantu meningkatkan terjemahan di Crowdin.",
                    thumbnail: CrowdinThumbnailUri,
        ...
        private void TryAddVanillaSource(string modDir)
        {
            try
            {
                ...
                if (allEntries == null)
                {
                    log.Warn("Failed to deserialize Vanilla/id-ID.json");
                    return;
                }
